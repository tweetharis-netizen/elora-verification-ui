...
// Core Libs
import { getSession, saveSession, seedDemoData } from "../../lib/session";
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        const s = getSession();
        if (!s || !s.email) {
            // Auto-seed for demo if no session exists
                        ...
                        <div className="flex flex-col items-center mb-12 gap-4">
                            <div className="bg-white dark:bg-neutral-900 p-1 rounded-2xl border-premium shadow-premium-sm flex gap-1">
                                {['student', 'parent', 'teacher'].filter(id => {
                                    if (isDemoMode) return true; // Allow all in demo
                            ...
                            {isDemoMode && (
                                <button 
                                    onClick={() => {
                                        if(confirm("Reset entire demo state? This will clear your changes.")) {
                                            const s = seedDemoData();
                                            updateSessionAndSync(s);
                                        }
                                    }}
                                    className="text-[10px] font-bold text-neutral-400 hover:text-rose-500 uppercase tracking-widest transition-colors"
                                >
                                    ↻ Reset Demo World
                                </button>
                            )}
