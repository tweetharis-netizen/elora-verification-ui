...
/**
 * Get classes for any user based on their role
 * @param {string} userId - The user's ID
 * @param {string} role - 'student', 'educator', or 'parent'
 * @param {string} linkedStudentId - Optional student ID for parents
 */
export async function getUserClasses(userId, role, linkedStudentId = null) {
    if (!db) return [];

    try {
        if (role === 'educator') {
            // Educators see classes they created
            return await getTeacherClasses(userId);
        } else if (role === 'student') {
            // Students see classes they've joined
            return await getStudentClasses(userId);
        } else if (role === 'parent' && linkedStudentId) {
            // Parents see their linked student's classes
            return await getStudentClasses(linkedStudentId);
        }
