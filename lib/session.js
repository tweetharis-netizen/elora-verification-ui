...
const DEFAULTS = {
  ...
  // Functional usage tracking
  usage: {
    messagesSent: 0,
    activeMinutes: 0,
    streak: 0,
    lastActive: null, // ISO string
    subjects: [], // array of strings
  },

  // Sharing code is persistent but generated ONLY for verified students.
  studentCode: null,
  linkedStudentId: null, // For individual parents

  // Educator specific
  linkedStudents: [], // Array of { code, name, stats: { messagesSent, activeMinutes, subjects } }
  classroom: {
    name: "My Classroom",
    totalEngagement: 0,
    classes: [], // Array of { id, name, subject, studentIds: [] }
  }
};
...
export function getSession() {
  ...
  // Robust merge for nested objects (usage, classroom)
  const merged = {
    ...DEFAULTS,
    ...(parsed || {}),
...
// NEW (Batch 14): switch roles without mixing assistant context
export function setRoleAndResetAssistant(role) {
  const s = getSession();
  const nextRole = String(role || "student");

  s.role = nextRole;

  // Reset assistant context so we never mix roles.
  s.messages = [];
  s.topicCustom = "";
  s.topicMode = "fixed";
  s.task = "tutor";
  ...
  // Educator should not be in guest preview mode.
  if (nextRole === "educator") s.guest = false;
