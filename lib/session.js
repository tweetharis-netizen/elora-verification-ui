...
/**
 * NEW (Batch 14): Switch role WITHOUT mixing chat context.
 * This resets assistant-specific fields so Student/Parent/Educator feels “real”.
 * - Does NOT touch theme/font prefs
 * - Does NOT touch verification/teacher flags (server-backed truth)
 */
export function setRoleAndResetAssistant(role) {
  ...
  // Reset assistant context so the model doesn't inherit the wrong persona/history.
  s.messages = [];
  s.topicCustom = "";
  // keep topicFixed as a safe default, but reset mode to fixed
  s.topicMode = "fixed";
  s.task = "tutor";
  s.style = "clear";

  // Educator should not be in guest preview mode (educator is verification-gated elsewhere).
