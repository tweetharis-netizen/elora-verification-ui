    ...
    return (
        <div className="bg-white rounded-2xl border border-[#EAE7DD] shadow-sm p-5 lg:p-6 transition-all duration-300">
            {!isExpanded ? (
                ...
                >
                    <div className={`${accentClasses.iconBg} w-7 h-7 rounded-lg border ${accentClasses.text} border-current flex items-center justify-center group-hover:scale-105 transition-transform`}>
                        <Sparkles className="w-4 h-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[13px] text-slate-800 font-semibold tracking-tight">💡 {title}</p>
                        <p className="text-[12px] text-slate-400 truncate font-medium">{description}</p>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-start gap-4">
                        <div className={`${accentClasses.iconBg} w-8 h-8 rounded-lg border ${accentClasses.text} border-current flex items-center justify-center mt-0.5 shrink-0`}>
                            <Sparkles className="w-4 h-4" />
                        </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 py-1">
                        {(suggestedPrompts.length > 0 ? suggestedPrompts : ['Quick Summary']).map((prompt) => (
                            <button
                                ...
                                className={`px-2.5 py-1 text-[11px] font-bold rounded-full border ${accentClasses.chipBg} ${accentClasses.text} border-current/10 hover:brightness-95 transition-all`}
                            >
                                {prompt}
                            </button>
                        ))}
                    ...
                    {effectiveStatus === 'idle' && <p className="text-xs text-slate-400">Ready to assist...</p>}
                    ...
                    {effectiveStatus === 'success' && effectiveSuggestion && (
                        <div className="space-y-3 bg-slate-50/50 rounded-2xl p-4 border border-slate-100/50">
                            <div>
                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">{effectiveSuggestion.kind.replace('_', ' ')}</p>
                                <h4 className="text-sm font-bold text-slate-900">{effectiveSuggestion.title}</h4>
                            </div>
                            <p className="text-[13px] text-slate-600 leading-relaxed font-medium">{effectiveSuggestion.body}</p>
                            ...
                            <div className="flex items-center justify-between pt-1">
                                ...
                                <span className="text-[10px] text-slate-300 font-medium">Assistant: {assistantLabel}</span>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="space-y-3 pt-3">
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                placeholder="Ask about learning progress..."
                                className="flex-1 px-3 py-2 text-xs border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-400/20 bg-slate-50/30 placeholder:text-slate-400"
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        e.preventDefault();
                            ...
                            <button
                                ...
                                className={`${accentClasses.buttonBg} text-white rounded-xl px-4 py-2 text-xs font-bold disabled:opacity-50 shadow-sm hover:brightness-110 transition`}
                            >
                                {isAsking ? '...' : 'Ask'}
                            </button>
                        </div>
                        ...
                        {customAnswer && (
                            <motion.div 
                                ...
                                className="px-4 py-3 bg-white border border-slate-100 rounded-2xl text-[13px] text-slate-700 whitespace-pre-wrap leading-relaxed shadow-sm font-medium"
                            >
                                {customAnswer}
                            </motion.div>
                        )}
                    ...
                    <button
                        ...
                        className="text-[11px] text-slate-400 hover:text-slate-600 font-bold uppercase tracking-wider mx-auto block pt-2"
                    >
                        Collapse
                    </button>
                </div>
