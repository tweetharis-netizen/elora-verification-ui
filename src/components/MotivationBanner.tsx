...
type MotivationVariant = 'info' | 'warning' | 'success';
...
const MotivationBanner: React.FC<MotivationBannerProps> = ({
    ...
    const variantClasses: Record<MotivationVariant, {
        gradient: string;
        accentIcon: string;
        pulseBadge: string;
        pillSurface: string;
        ...
        info: {
            gradient: 'from-[var(--elora-secondary)] via-[var(--elora-primary)] to-[var(--elora-deep)]',
            accentIcon: 'text-[var(--elora-info-bg)]',
            pulseBadge: 'bg-[var(--elora-primary)] text-white',
            pillSurface: 'bg-[var(--elora-info-bg)]/20 border-[var(--elora-info-bg)]/40',
        },
        warning: {
            gradient: 'from-amber-500 via-orange-400 to-[var(--elora-warning-bg)]',
            accentIcon: 'text-[var(--elora-warning-bg)]',
            pulseBadge: 'bg-[var(--elora-warning-bg)] text-[var(--elora-text-strong)]',
            pillSurface: 'bg-[var(--elora-warning-bg)]/25 border-[var(--elora-warning-bg)]/45',
        },
        success: {
            gradient: 'from-emerald-500 via-teal-500 to-[var(--elora-primary)]',
            accentIcon: 'text-emerald-200',
            pulseBadge: 'bg-emerald-500 text-white',
            pillSurface: 'bg-emerald-100/25 border-emerald-100/45',
        },
    };
    ...
    return (
        <section className={`relative overflow-hidden rounded-2xl border border-[var(--elora-border-subtle)] shadow-lg mb-8 group ${className ?? ''}`.trim()}>
            {/* Background Gradient & Pattern */}
            <div className={`absolute inset-0 bg-gradient-to-br ${palette.gradient} opacity-95 group-hover:scale-105 transition-transform duration-700`}></div>
            ...
            <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 text-white">
                <div className="flex-1 space-y-4">
                    <div className="flex flex-wrap items-center gap-3">
                        {/* Streak Badge */}
                        <div className={`flex items-center gap-2 px-3 py-1.5 backdrop-blur-md rounded-full border shadow-sm animate-pulse ${palette.pillSurface}`}>
                ...
                <div className="hidden lg:block">
                    <div className="w-24 h-24 bg-white/10 backdrop-blur-xl rounded-2xl rotate-12 flex items-center justify-center border border-white/20 shadow-2xl relative">
                        <Star className={`w-12 h-12 fill-current animate-bounce ${palette.accentIcon}`} />
                        <div className={`absolute -top-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shadow-lg border-2 border-white ${palette.pulseBadge}`}>
                            HOT
                        </div>
                    </div>
                </div>
